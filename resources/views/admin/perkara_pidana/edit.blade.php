					...
					<div class="row">
						<div class="col-12 col-xl-12">
							<div class="card">
								...
								<div class="card-body">
									<form action="{{ url('/'.Request::segment(1).'/edit/'.$perkara_pidana->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
									{{ csrf_field() }}
									<input type="hidden" name="_method" value="PUT">
									<div class="form-group row">
											<label class="col-form-label col-sm-3 text-sm-right"> Judul <span class="required" style="color: #dd4b39;">*</span></label>
											<div class="col-sm-9">
												<input type="text" name="judul" class="form-control @if ($errors->has('judul')) is-invalid @endif " value="{{ $perkara_pidana->judul }}">
										...
										<div class="form-group row">
											...
											<div class="col-sm-9">
												<textarea name="deskripsi" class="form-control @if ($errors->has('deskripsi')) is-invalid @endif ">{{ $perkara_pidana->deskripsi }}</textarea>
											...
											<div class="col-sm-9">
												<select name="tahun" class="form-control @if ($errors->has('tahun')) is-invalid @endif ">
													...
													@for($i=2019;$i<=date('Y');$i++)
														<option value="{{ $i }}" @if($perkara_pidana->tahun==$i) selected @endif> {{ $i }}</option>
													@endfor
												</select>
										...
										<div class="form-group row">
											...
											<div class="col-sm-9">
												...
												@if($perkara_pidana->arsip_litigasi)
													<br>
													<a href="{{ asset('upload/arsip_litigasi/'.$perkara_pidana->arsip_litigasi) }}" target="_blank" class="btn btn-sm btn-primary" >Lihat File</a>
												@endif
											</div>
										</div>
										<br><br>
