					...
					<div class="row">
						<div class="col-12 col-xl-12">
							<div class="card">
								...
								<div class="card-body">
									<form action="{{ url('/'.Request::segment(1).'/edit/'.$pengaduan_hukum->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
									{{ csrf_field() }}
									<input type="hidden" name="_method" value="PUT">
									<div class="form-group row">
											<label class="col-form-label col-sm-3 text-sm-right"> Judul <span class="required" style="color: #dd4b39;">*</span></label>
											<div class="col-sm-9">
												<input type="text" name="judul" class="form-control @if ($errors->has('judul')) is-invalid @endif " value="{{ $pengaduan_hukum->judul }}">
										...
										<div class="form-group row">
											...
											<div class="col-sm-9">
												<textarea name="deskripsi" class="form-control @if ($errors->has('deskripsi')) is-invalid @endif ">{{ $pengaduan_hukum->deskripsi }}</textarea>
											...
											<div class="col-sm-9">
												<select name="tahun" class="form-control @if ($errors->has('tahun')) is-invalid @endif ">
													...
													@for($i=2019;$i<=date('Y');$i++)
														<option value="{{ $i }}" @if($pengaduan_hukum->tahun==$i) selected @endif> {{ $i }}</option>
													@endfor
												</select>
										...
										<div class="form-group row">
											<label class="col-form-label col-sm-3 text-sm-right">Berkas Arsip Pengaduan Hukum <span class="required" style="color: #dd4b39;">*</span></label>
											<div class="col-sm-9">
												...
												@if($pengaduan_hukum->arsip_non_litigasi)
													<br>
													<a href="{{ asset('upload/arsip_non_litigasi/'.$pengaduan_hukum->arsip_non_litigasi) }}" target="_blank" class="btn btn-sm btn-primary" >Lihat File</a>
												@endif
											</div>
										</div>
										<br><br>
